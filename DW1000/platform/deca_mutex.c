...
decaIrqStatus_t decamutexon(void)
{
    ...
    decaIrqStatus_t decaIrqStatus = port_GetEXT_IRQStatus();
    ...
    return decaIrqStatus; // return state before disable, value is used to re-enable in decamutexoff call
}
 ...
 *
 * input parameters:
 * @param decaIrqStatus - the state of the DW1000 interrupt as returned by decamutexon
 *
 * output parameters
 *
 * returns the state of the DW1000 interrupt
 */
void decamutexoff(decaIrqStatus_t decaIrqStatus) // put a function here that re-enables the interrupt at the end of the critical section
{
    if (decaIrqStatus)
