 ...
 * @param delta_temp - int - the difference between current ambient temperature (raw value units) and the temperature at which reference measurements were made (raw value units)
 * output parameters: None
 *
 ...
 *
 * input parameters:
 * @param head_octets_number  - number of bytes header being written
 * @param head_octets_buffer  - pointer to buffer containing the 'head_octets_number' bytes of header to be written
 * @param bodylength    - number of bytes data being written
 * @param body_octets_buffer    - pointer to buffer containing the 'bodylength' bytes od data to be written
 *
 * output parameters
 *
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int writetospi(uint16_t head_octets_number, const uint8_t* head_octets_buffer, uint32_t bodylength, const uint8_t* body_octets_buffer);
 ...
 *
 * input parameters:
 * @param head_octets_number - number of bytes header to write
 * @param head_octets_buffer - pointer to buffer containing the 'head_octets_number' bytes of header to write
 * @param body_octets_number - number of bytes data being read
 * @param body_octets_buffer - pointer to buffer containing to return the data (NB: size required = head_octets_number + body_octets_number)
 *
 * output parameters
 *
...
typedef uint32_t decaIrqStatus_t; // Type for remembering IRQ status
 ...
 * returns the state of the DW1000 interrupt
 */
void decamutexoff(decaIrqStatus_t decaIrqStatus);
