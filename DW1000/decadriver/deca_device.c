...
int dwt_initialise(int config)
{
    ...
    if ((!(DWT_DW_WAKE_UP & config)) || ((DWT_DW_WAKE_UP & config) && (DWT_DW_WUP_RD_OTPREV & config)))
    {
        ...
        otp_xtaltrim_and_rev = _dwt_otpread(XTRIM_ADDRESS) & 0xFFFFU;   // Read 32 bit value, XTAL trim val is in low octet-0 (5 bits)
        pdw1000local->otprev = (otp_xtaltrim_and_rev >> 0x08U) & 0xFFU; // OTP revision is the next byte
    }
    else
    ...
    {
        // Load VBAT from OTP
        pdw1000local->vBatP = _dwt_otpread(VBAT_ADDRESS) & 0xFFU;
    }
    else
    {
    ...
    {
        // Load TEMP from OTP
        pdw1000local->tempP = _dwt_otpread(VTEMP_ADDRESS) & 0xFFU;
    }
    else
    {
...
void dwt_setgpiodirection(uint32_t gpioNum, uint32_t direction)
{
    ...
    buf[0] = command & 0xFFU;
    buf[1] = (command >> 8) & 0xFFU;
    buf[2] = (command >> 16) & 0xFFU;
 ...
 * no return value
 */
void dwt_writetodevice(uint8_t recordNumber, uint16_t index, uint32_t length, const uint8_t* buffer)
{
    ...
    {
        header[cnt++] = 0x80U | recordNumber; // Bit-7 is WRITE operation, bit-6 zero=NO sub-addressing, bits 5-0 is reg file id
    }
    else
    {
        ...
        header[cnt++] = 0xC0U | recordNumber; // Bit-7 is WRITE operation, bit-6 one=sub-address follows, bits 5-0 is reg file id
 ...
 * no return value
 */
void dwt_readfromdevice(uint8_t recordNumber, uint16_t index, uint32_t length, uint8_t* buffer)
{
 ...
 */
uint32_t dwt_read32bitoffsetreg(uint8_t regFileID, uint16_t regOffset)
{
    uint32_t regval = 0;
    int j;
    uint8_t buffer[4];
 ...
 */
uint16_t dwt_read16bitoffsetreg(uint8_t regFileID, uint16_t regOffset)
{

    uint8_t buffer[2];

    dwt_readfromdevice(regFileID, regOffset, 2, buffer);

    uint16_t regval = (uint16_t)(((uint16_t)buffer[1] << 8) | buffer[0]);
 ...
 */
uint8_t dwt_read8bitoffsetreg(uint8_t regFileID, uint16_t regOffset)
{
    uint8_t regval;
 ...
 * no return value
 */
void dwt_write8bitoffsetreg(uint8_t regFileID, uint16_t regOffset, uint8_t regval)
{
 ...
 * no return value
 */
void dwt_write16bitoffsetreg(uint8_t regFileID, uint16_t regOffset, uint16_t regval)
{
    uint8_t buffer[2];
    buffer[0] = (uint8_t)(regval & 0xFFU);
    buffer[1] = (uint8_t)(regval >> 0x08U);
 ...
 * no return value
 */
void dwt_write32bitoffsetreg(uint8_t regFileID, uint16_t regOffset, uint32_t regval)
{
    int j;
    uint8_t buffer[4];
...
void dwt_otpread(uint16_t address, uint32_t* array, uint8_t length)
{
    ...
    for (uint8_t i = 0x00U; i < length; i++)
    {
...
uint32_t _dwt_otpprogword32(uint32_t data, uint16_t address)
{
    ...
    wr_buf[3] = (uint8_t)((data >> 24) & 0xFFU);
    wr_buf[2] = (uint8_t)((data >> 16) & 0xFFU);
    wr_buf[1] = (uint8_t)((data >> 8) & 0xFFU);
    wr_buf[0] = (uint8_t)((data >> 0) & 0xFFU);

    dwt_writetodevice(OTP_IF_ID, OTP_WDAT, 4, wr_buf);

    // Write the address [10:0]
    wr_buf[1] = (address >> 8) & 0x07;
    wr_buf[0] = address & 0xffU;
...
void dwt_setrxtimeout(uint16_t time)
{
    ...
    {
        temp &= (uint8_t)(~((uint8_t)(SYS_CFG_RXWTOE >> 24))); // Shift RXWTOE mask as we read the upper byte only
...
float dwt_convertrawtemperature(uint8_t raw_temp)
{
    ...
    realtemp = ((raw_temp - pdw1000local->tempP) * SAR_TEMP_TO_CELCIUS_CONV) + 23.0f;
...
uint8_t dwt_convertvoltstoraw(int32_t externalmvolt)
{
    ...
    double temp = ((double)(externalmvolt - 3300)) * MVOLT_TO_SAR_VBAT_CONV;
    raw_voltage = (uint32_t)(temp + 0.5); // simple rounding mechanism instead of truncation
    raw_voltage += pdw1000local->vBatP;
 ...
 * @param power_adj - int32_t - the adjustment in power level to be made, in 0.5dB steps
 *
 * output parameters:
 *
...
uint32_t _dwt_computetxpowersetting(uint32_t ref_powerreg, int32_t power_adj)
{
    ...
    int8_t mixer_gain_change;

    uint8_t current_da_attn;
    uint8_t current_mixer_gain;

    uint8_t new_da_attn;
    uint8_t new_mixer_gain;
...
uint16_t dwt_calcpgcount(uint8_t pgdly)
{
    ...
    for (uint8_t i = 0x00U; i < NUM_SAMPLES; i++)
    {
        // Write bw setting to PG_DELAY register
